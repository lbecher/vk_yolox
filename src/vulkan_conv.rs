...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    if (col == 0u) {
        let idx = x;
        value = (sigmoid(input_buffer.data[idx]) + f32(x)) * f32(stride);
    } else if (col == 1u) {
        let idx = (input_h * input_w) + (y * input_w) + x;
        value = (sigmoid(input_buffer.data[idx]) + f32(y)) * f32(stride);
    } else if (col == 2u) {
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let patch = out_channel / input_channels;
...
pub fn run_demo_decode(
    ...
    let backbone_outputs = runtime.run_backbone(&input, backbone)?;
    let pafpn_outputs = runtime.run_pafpn(&backbone_outputs, pafpn)?;
    let head_outputs = runtime.run_head(&pafpn_outputs, head)?;
    let decoded = runtime.run_decode(&head_outputs, decode)?;

    Ok(DecodedPredictions {
        rows: decoded.rows,
        cols: decoded.cols,
        data: runtime.read_matrix(&decoded)?,
    })
}
...
pub fn run_demo_decode_resident(
    ...
    let runtime = VulkanRuntime::new(device_index)?;
    let input = runtime.upload_tensor(input, input_shape)?;
    let backbone = runtime.prepare_backbone(backbone)?;
    let pafpn = runtime.prepare_pafpn(pafpn)?;
    let head = runtime.prepare_head(head)?;

    let backbone_outputs = runtime.run_backbone_prepared(&input, &backbone)?;
    let pafpn_outputs = runtime.run_pafpn_prepared(&backbone_outputs, &pafpn)?;
    let head_outputs = runtime.run_head_prepared(&pafpn_outputs, &head)?;
    let decoded = runtime.run_decode(&head_outputs, decode)?;
...
impl VulkanRuntime {
    ...
    fn read_tensor(&self, tensor: &GpuTensor) -> Result<Vec<f32>> {
        ...
        self.copy_buffer_and_wait(tensor.buffer.clone(), readback.clone())?;
        let output = readback
            .read()
    ...
    fn read_matrix(&self, matrix: &GpuMatrix) -> Result<Vec<f32>> {
        ...
        self.copy_buffer_and_wait(matrix.buffer.clone(), readback.clone())?;
        let output = readback
            .read()
    ...
    fn run_conv2d_prepared(
        ...
        let output_buffer =
            self.create_device_buffer::<f32>(output_shape.len(), BufferUsage::STORAGE_BUFFER)?;
        let params = [
        ...
        let params_buffer = self.upload_device_buffer(&params, BufferUsage::STORAGE_BUFFER)?;
    ...
    fn run_silu_tensor(&self, input: &GpuTensor) -> Result<GpuTensor> {
        let output_buffer =
            self.create_device_buffer::<f32>(input.shape.len(), BufferUsage::STORAGE_BUFFER)?;
    ...
    fn run_add_tensors(&self, lhs: &GpuTensor, rhs: &GpuTensor) -> Result<GpuTensor> {
        ...
        let output_buffer =
            self.create_device_buffer::<f32>(lhs.shape.len(), BufferUsage::STORAGE_BUFFER)?;
    ...
    fn run_baseconv_block(&self, input: &GpuTensor, block: &BaseConvBlock) -> Result<GpuTensor> {
        let conv = self.run_conv2d_tensor(input, &block.conv)?;
        self.run_silu_tensor(&conv)
    }
    ...
    fn run_baseconv_prepared(
        ...
        let conv = self.run_conv2d_prepared(input, &block.conv)?;
        self.run_silu_tensor(&conv)
    }
    ...
    fn run_dwsconv_block(&self, input: &GpuTensor, block: &DwsConvBlock) -> Result<GpuTensor> {
        let depthwise = self.run_conv2d_tensor(input, &block.depthwise)?;
        let depthwise = self.run_silu_tensor(&depthwise)?;
        let pointwise = self.run_conv2d_tensor(&depthwise, &block.pointwise)?;
        self.run_silu_tensor(&pointwise)
    }
    ...
    fn run_dws_prepared(
        ...
        let depthwise = self.run_conv2d_prepared(input, &block.depthwise)?;
        let depthwise = self.run_silu_tensor(&depthwise)?;
        let pointwise = self.run_conv2d_prepared(&depthwise, &block.pointwise)?;
        self.run_silu_tensor(&pointwise)
    }
    ...
    fn run_csp_bottleneck_prepared(
        ...
        for bottleneck in &block.blocks {
            x1 = self.run_bottleneck_prepared(&x1, bottleneck)?;
        }

        let cat = self.run_concat_channels_tensor(&x1, &x2)?;
        self.run_baseconv_prepared(&cat, &block.conv3)
    }
    ...
    fn run_csp_stage_prepared(
        ...
        self.run_csp_bottleneck_prepared(&x, &block.c3)
    }
    ...
    fn run_spp_bottleneck_block(
        ...
        let p0 = self.run_maxpool2d_tensor(&base, block.pooling[0], 1, block.pooling[0] / 2)?;
        let p1 = self.run_maxpool2d_tensor(&base, block.pooling[1], 1, block.pooling[1] / 2)?;
        let p2 = self.run_maxpool2d_tensor(&base, block.pooling[2], 1, block.pooling[2] / 2)?;

        let cat01 = self.run_concat_channels_tensor(&base, &p0)?;
        let cat012 = self.run_concat_channels_tensor(&cat01, &p1)?;
        let cat = self.run_concat_channels_tensor(&cat012, &p2)?;
    ...
    fn run_spp_bottleneck_prepared(
        ...
        self.run_baseconv_prepared(&cat, &block.conv2)
    }
    ...
    fn run_dark5_prepared(
        ...
        let x = self.run_conv_prepared(input, &block.conv)?;
        let x = self.run_spp_bottleneck_prepared(&x, &block.spp)?;
        self.run_csp_bottleneck_prepared(&x, &block.c3)
    }
    ...
    fn run_head_scale_block(
        ...
        let reg_obj = self.run_concat_channels_tensor(&reg_pred, &obj_pred)?;
        self.run_concat_channels_tensor(&reg_obj, &cls_pred)
    }
    ...
    fn run_head_scale_prepared(
        ...
        let cls = self.run_conv_prepared(&cls, &head.cls_conv2)?;
        let cls_pred = self.run_conv2d_prepared(&cls, &head.cls_pred)?;

        let reg = self.run_conv_prepared(&stem, &head.reg_conv1)?;
        let reg = self.run_conv_prepared(&reg, &head.reg_conv2)?;
        let reg_pred = self.run_conv2d_prepared(&reg, &head.reg_pred)?;
        let obj_pred = self.run_conv2d_prepared(&reg, &head.obj_pred)?;
    ...
    fn run_decode_head_scale(
        ...
        let output_buffer =
            self.create_device_buffer::<f32>(rows * cols, BufferUsage::STORAGE_BUFFER)?;
        let params = [
            input.shape.channels as u32,
            input.shape.height as u32,
            input.shape.width as u32,
    ...
    fn run_decode(&self, head: &GpuHeadFeatures, decode: &YoloxDecodeDemo) -> Result<GpuMatrix> {
        let s8 = self.run_decode_head_scale(&head.s8, 8, decode.num_classes)?;
        let s16 = self.run_decode_head_scale(&head.s16, 16, decode.num_classes)?;
        let s32 = self.run_decode_head_scale(&head.s32, 32, decode.num_classes)?;

        let s8_s16 = self.run_concat_rows(&s8, &s16)?;
        self.run_concat_rows(&s8_s16, &s32)
    }
    ...
    fn execute_compute(
        ...
        command_buffer
            ...
            .with_context(|| format!("falha ao flushar `{label}`"))?
            .wait(None)
            .with_context(|| format!("falha ao aguardar `{label}`"))
    }
    ...
    >(
        ...
        self.copy_buffer_and_wait(staging_buffer, device_buffer.clone())?;
        Ok(device_buffer)
    }
